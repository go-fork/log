...
## Download
- Source code: [go.fork.vn/log@$target_version]
- Documentation: [pkg.go.dev/go.fork.vn/log@$target_version]
...
\`\`\`bash
go get go.fork.vn/log@$target_version
go mod tidy
\`\`\`
...
## Getting Help
- Check the [documentation](https://pkg.go.dev/go.fork.vn/log@$target_version)
